...
function rangeSliderUpdate(e) {
  ...
  // Pts out of 100
  //var updatedPts = Math.floor(points / 8.5);
  // Show new pts value
  //range.find(".count").text(updatedPts).val(updatedPts);
  prevVal = deg;
...
function rangeSliderUpdate(e) {
  ...
  // Pts out of 100
  var updatedPts = Math.floor(points / 8.5);
  var rangeId = range.attr("id");
  // Show new pts value
  range.find(".count").text(updatedPts).val(updatedPts);
  $("#item_"+rangeId).find(".range-count").text(updatedPts).val(updatedPts);
  prevVal = deg;
  lastPosition = position.x;
  callAjax(updatedPts, rangeId);
}
